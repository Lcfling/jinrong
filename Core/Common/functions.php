...
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由
            ...
            if((!C('APP_SUB_DOMAIN_DEPLOY') || in_array(GROUP_NAME, array('Admin','Shangjia','Developer','Mobile','Store','App','Wuye','Mcenter','Member')))  && C('APP_GROUP_LIST')) {
                if(!empty($path)) {
    ...
    if($redirect) // 直接跳转URL
        ...
        return preg_replace('/^\/pchome/i', '', $url);
}
